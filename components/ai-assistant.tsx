...
export function AiAssistant() {
  ...
  const DISPLAY_NAME = PUBLIC_SAFE_MODE ? "Assistant" : "Momo"
  ...
  const isBookingOrAvailabilityIntent = (text: string): boolean => {
    if (PUBLIC_SAFE_MODE) return false
    const lower = text.toLowerCase()
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role:"assistant", content:`Booked! \u2713 ${bookingDate} ${bookingSlot.label}. Looking forward to it!`, type:"booking-confirmed" }])
  ...
  const handleSend = async () => {
    ...
    if (isBookingOrAvailabilityIntent(text)) {
      setMessages(prev => [...prev, { id:(Date.now()+1).toString(), role:"assistant", content: PUBLIC_SAFE_MODE ? "Booking is not available in anonymous mode." : "I'd love to meet! What date works for you? (e.g. \"March 10\", \"next Tuesday\")" }])
      return
    }
    if (date) {
  ...
  const startCall = () => {
    ...
    speak(`Hey! I'm ${DISPLAY_NAME}. What can I help you with?`, () => { setCallStatus(""); startListening() })
  }
            ...
            <div className="flex flex-col items-center justify-center py-10 gap-4" style={{ minHeight: '420px', background: isCallActive ? 'linear-gradient(180deg, oklch(0.22 0.02 155) 0%, oklch(0.16 0.015 160) 100%)' : undefined }}>
              ...
              {!isCallActive ? (
                ...
                <div className="flex flex-col items-center gap-5 text-center w-full px-6">
                  ...
                  <div>
                    <h4 className="text-lg font-semibold text-white">{DISPLAY_NAME}</h4>
                    ...
                    <div className="w-full max-h-32 overflow-y-auto rounded-xl bg-white/5 backdrop-blur-sm px-4 py-3">
                      {messages.slice(-4).map(msg => (
                        <div key={msg.id} className={`text-xs mb-1 ${msg.role === "user" ? "text-right text-white/80" : "text-left text-white/50"}`}>
                          <span className="font-semibold">{msg.role === "user" ? "You" : DISPLAY_NAME}:</span>{" "}
